...
const findPath = (
  ...
  let unvisited: TState[] = [startState];
...
const readMap = () => {
  const input = readInput().filter((i: string) => i);
  ...
  findPath(
    tiles[0][0],
    tiles[tiles.length - 1][tiles[0].length - 1],
    tiles,
    0,
    3
  );
