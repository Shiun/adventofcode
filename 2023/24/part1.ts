...
const parseHail = (line: string): Hail => {
  const regex =
    /^(?<x>[-\d]+)[, ]+(?<y>[-\d]+)[, ]+(?<z>[-\d]+)[@ ]+(?<vx>[-\d]+)[, ]+(?<vy>[-\d]+)[, ]+(?<vz>[-\d]+)$/;
...
const hasIntersection = (h1: Hail, h2: Hail, min: number, max: number) => {
  let m1 = h1.vy/h1.vx
  let m2 = h2.vy/h2.vx
...
const countIntersections = (hails: Hail[], min: number, max: number) => {
...
const readHail = () => {
  ...
  let hails = lines.map((l: string) => parseHail(l))
