...
const transposePlatform = (lines: Array<Array<string>>) => {
  const rows = lines.length, cols = lines[0].length;
  let tPlatform: Array<Array<string>> = [];
...
const tiltPlatform = (lines: Array<Array<string>>) => {
...
const calculateLoad = (lines: Array<Array<string>>) => {
  let sum = 0
  lines.forEach((r, i) => {
    r.forEach((c, j) => {
      if(c === 'O') {
...
const printPlatform = (map: Array<Array<string>>) => {
  map.forEach(r => {
    console.log(r.reduce((s, c) => s + c, ""))
  })
}

const spinPlatform = (map: Array<Array<string>>, count: number, tilt: boolean = true) => {
  let tPlatform = map
  let sums: number[] = []
