...
const boundIndex = (index: number, min: number, max: number) => {
  if (index < min) {
    return min;
  }
  if (index > max) {
...
const hasSymbols = (line: Array<string>, start: number, stop: number) => {
...
const isPartsNumber = (
  ...
  if (lineIndex - 1 >= 0) {
    ...
    hasSymbolsAbove = hasSymbols(
      lines[lineIndex - 1],
      boundIndex(start - 1, 0, lines[lineIndex - 1].length - 1),
      boundIndex(end + 1, 0, lines[lineIndex - 1].length - 1)
    );
    if (hasSymbolsAbove) {
      return test;
    }
...
const inputStringArray = () => {
  ...
  twoDMap.forEach((line, lineIndex) => {
    ...
    while (
      ...
      if (!isNaN(result)) {
        sumOfPartsNumber += isPartsNumber(
          result,
          lines,
          lineIndex,
          indexes.start,
