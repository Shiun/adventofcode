...
const numberLocations = (map) => {
    let numberMap = Array(map.length);
...
const findNumbers = (map) => {
    const numbers = numberLocations(map);
    return numbers;
};
const limitIndex = (max, min, index) => {
    if (index < min) {
        return min;
    }
    if (index > max) {
...
const gearParimeter = (max, min, gear) => {
