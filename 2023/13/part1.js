...
const checkVerticalReflectionIndex = (pattern, index) => {
    ...
    for (let row = 0; row < pattern.length && isReflection; row++) {
        ...
        for (let i = index, j = index + (index - i) + 1; i >= 0 && j < rowLength && isReflection; i--, j++) {
            // console.log('i, j', i, j)
...
const findVerticalReflectionIndex = (pattern) => {
    ...
    potentials.forEach((i) => {
        if (vIndex === -1)
            vIndex = checkVerticalReflectionIndex(pattern, i);
    });
    return vIndex;
};
const findVerticalReflectionCount = (patterns) => {
    let columns = [];
    patterns.forEach(p => {
...
const checkHorizontalReflectionIndex = (pattern, index) => {
    // console.log('pattern.length', pattern.length)
    let isReflection = true;
    for (let col = 0; col < pattern[index].length && isReflection; col++) {
        for (let i = index, j = index + (index - i) + 1; i >= 0 && j < pattern.length && isReflection; i--, j++) {
...
const findHorizontalReflectionIndex = (pattern) => {
    let potentials = [];
    ...
    potentials.forEach((i) => {
        if (hIndex === -1)
            hIndex = checkHorizontalReflectionIndex(pattern, i);
    });
    return hIndex;
};
const findHorizontalReflectionCount = (patterns) => {
    let rows = [];
    patterns.forEach(p => {
